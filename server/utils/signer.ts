...
import { CertificateProcessor, CertificateInfo, MobileProvision } from './certificate';
...
export class IPASigner {
   ...
   */
  async signIPA(files: SigningJobFiles, params: SigningJobParams): Promise<SigningResult> {
    ...
    try {
      ...
      const provisionInfo = await CertificateProcessor.processMobileProvision(files.mpFile);
      
      // Step 2: Validate certificate and provision compatibility
      console.log(`[SIGNER] Validating certificate and provision...`);
      const isCompatible = CertificateProcessor.validateCertificateProvisionMatch(
        certificateInfo, 
        provisionInfo
      );
      
      if (!isCompatible) {
        console.warn(`[SIGNER] Certificate and provision may not be compatible, but continuing...`);
      }
      ...
      if (params.entitlements) {
        try {
          ...
          console.warn('[SIGNER] Could not parse custom entitlements as JSON, ignoring');
        }
      }
      ...
      const entitlementsPath = path.join(extractedDir, 'Payload', fs.readdirSync(path.join(extractedDir, 'Payload'))[0], 'entitlements.plist');
      await IPAProcessor.signBinaries(extractedDir, certificateInfo, entitlementsPath);
      
      // Step 9: Create signed IPA
      console.log(`[SIGNER] Creating signed IPA...`);
      const signedIpaPath = path.join(this.outputDir, `${originalInfo.bundleName || 'signed'}-signed.ipa`);
      ...
      console.log(`[SIGNER] Original: ${originalInfo.bundleName} (${originalInfo.bundleId})`);
      console.log(`[SIGNER] Signed IPA: ${signedIpaPath} (${(signedIpaSize / 1024 / 1024).toFixed(2)} MB)`);
  ...
  private async downloadIPA(url: string): Promise<string> {
    try {
      ...
      console.log(`[SIGNER] Downloaded IPA: ${(buffer.length / 1024 / 1024).toFixed(2)} MB`);
      return ipaPath;
    } catch (error) {
      console.error('[SIGNER] Error downloading IPA:', error);
      throw new Error(`Failed to download IPA: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  private async processAdditionalFiles(extractedDir: string, files: SigningJobFiles): Promise<void> {
    try {
      const payloadDir = path.join(extractedDir, 'Payload');
      const appDirs = fs.readdirSync(payloadDir).filter(name => name.endsWith('.app'));
      const appDir = path.join(payloadDir, appDirs[0]);
      
      // Process cyan files
      if (files.cyanFiles?.length) {
        console.log(`[SIGNER] Processing ${files.cyanFiles.length} cyan files...`);
        for (const cyanFile of files.cyanFiles) {
          ...
          console.log(`[SIGNER] Processing cyan file: ${path.basename(cyanFile)}`);
        }
      }
      
      // Process tweak files (dylibs, frameworks, bundles)
      if (files.tweakFiles?.length) {
        console.log(`[SIGNER] Processing ${files.tweakFiles.length} tweak files...`);
      ...
      throw new Error(`Failed to process additional files: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  cleanup(): void {
    try {
      if (fs.existsSync(this.workingDir)) {
        ...
        console.log(`[SIGNER] Cleaned up working directory: ${this.workingDir}`);
      }
    } catch (error) {
