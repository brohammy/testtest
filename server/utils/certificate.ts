...
export class CertificateProcessor {
  ...
  static async processP12Certificate(
    ...
    try {
      ...
      if (!certificate || !privateKey) {
        throw new Error('Could not extract certificate or private key from P12 file');
      }
      ...
      throw new Error(`Failed to process P12 certificate: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  static async processMobileProvision(mpFilePath: string): Promise<MobileProvision> {
    try {
      ...
      const extractValue = (key: string): any => {
        const keyRegex = new RegExp(`<key>${key}</key>\\s*<([^>]+)>([^<]*)</\\1>`);
        const match = plistContent.match(keyRegex);
        return match ? match[2] : null;
      };
      
      const extractArray = (key: string): string[] => {
        const keyRegex = new RegExp(`<key>${key}</key>\\s*<array>([\\s\\S]*?)</array>`);
        const match = plistContent.match(keyRegex);
        if (!match) return [];
      ...
      throw new Error(`Failed to process mobile provision: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static validateCertificateProvisionMatch(
    ...
    if (cert.teamId !== provision.teamId) {
      console.warn(`Team ID mismatch: Certificate=${cert.teamId}, Provision=${provision.teamId}`);
      return false;
    }
