...
export class IPAProcessor {
   ...
   */
  static async extractIPA(ipaPath: string, outputDir: string): Promise<IPAInfo> {
    try {
      ...
      const bundleName = infoPlist.CFBundleDisplayName || infoPlist.CFBundleName || '';
      const bundleVersion = infoPlist.CFBundleVersion || '1.0';
      const bundleShortVersion = infoPlist.CFBundleShortVersionString || bundleVersion;
      ...
      throw new Error(`Failed to extract IPA: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static async modifyIPA(
    ...
    try {
      ...
      if (options.minimumOSVersion) {
        ...
        console.log(`[IPA] Changed minimum iOS version to: ${options.minimumOSVersion}`);
      }
      ...
      throw new Error(`Failed to modify IPA: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static async installProvisionAndEntitlements(
    ...
    try {
      ...
      const embeddedProvisionPath = path.join(appDir, 'embedded.mobileprovision');
      ...
      const entitlements = customEntitlements || provision.entitlements || {
      ...
      throw new Error(`Failed to install provision: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   * Create signed IPA
   */
  static async createSignedIPA(extractedDir: string, outputPath: string): Promise<void> {
    try {
      ...
      throw new Error(`Failed to create signed IPA: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static async signBinaries(
    ...
    try {
      console.log(`[IPA] Signing binaries with certificate: ${certificate.commonName}`);
      ...
      throw new Error(`Failed to sign binaries: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
